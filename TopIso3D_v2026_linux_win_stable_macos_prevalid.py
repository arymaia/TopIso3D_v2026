...
def _show_plotly_figure(fig, *, saved_html: Path | None = None) -> None:
    """Open a Plotly figure in a browser using a macOS-friendly fallback path."""
    ...
    tmp = Path(tempfile.gettempdir()) / f"topiso3d_plotly_{os.getpid()}_{int(time.time() * 1000)}.html"
    fig.write_html(str(tmp), include_plotlyjs=True, auto_open=False)
    if not _open_file_uri_in_browser(tmp):
        raise RuntimeError("Could not open Plotly figure in the system browser.")
...
class App(tk.Tk):
    def __init__(self):
        ...
        _ensure_floating_window(self)
        self.title("TopIso3D v2026 - macOS pre-validation v5")
        ...
        self.after(self._ui_refresh_ms, self.refresh_ui_state)
    ...
    def _about(self):
        messagebox.showinfo("About", "TopIso3D v2026 macOS pre-validation v5: visual refinement + real-Mac test preparation build.")
...
def _run_pl2d(self):
    ...
    # Run slices sequentially (official Windows-local mode and safe default elsewhere)
    ...
    for i, z in enumerate(zs):
        ...
        # Cleanup according to user policy. Ignore failures.
